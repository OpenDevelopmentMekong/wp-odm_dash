...
class wpdash_Query_Resources_Widget extends WP_Widget
{
 ...
 public function __construct() {
  parent::__construct(
   ...
   __('wpdash Query Datasets', 'wpdash'),
   array('description' => __('Query resources and displays them in a post or page.', 'wpdash'))
  );
 }
  ...
  global $post;
   ...
   if (!empty($output) && $output != ""):
     ...
     if ( ! empty( $instance['title'] ) ) :
      echo $args['before_title'] . apply_filters( 'widget_title', __( $instance['title'], 'wpdash')). $args['after_title'];
     endif;
 ...
 public function form( $instance ) {
  // outputs the options form on admin
  $title = ! empty( $instance['title'] ) ? __( $instance['title'], 'wpdash') : __( 'Related datasets', 'wpdash' );
  ...
  <p>
   ...
   <select class="widefat" id="<?php echo $this->get_field_id( 'organization' ); ?>" name="<?php echo $this->get_field_name( 'organization' ); ?>">
      <option <?php if($organization == -1) echo 'selected="selected"' ?> value="-1"><?php _e('All','wpdash')?></option>
   ...
   <select class="widefat" id="<?php echo $this->get_field_id( 'group' ); ?>" name="<?php echo $this->get_field_name( 'group' ); ?>">
      <option <?php if($group == -1) echo 'selected="selected"' ?> value="-1"><?php _e('All','wpdash')?></option>
