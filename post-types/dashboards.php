...
if (!class_exists('Odm_Dashboards_Post_Type')) {
    ...
    class Odm_Dashboards_Post_Type
    {
        ...
        {
            $labels = array(
            'name' => __('Dashboards', 'post type general name', 'odi'),
            'singular_name' => __('Dashboard', 'post type singular name', 'odi'),
            'menu_name' => __('Dashboards', 'admin menu for dashboard', 'odi'),
            'name_admin_bar' => __('Dashboards', 'add new on admin bar', 'odi'),
            'add_new' => __('Add new', 'dashboard', 'odi'),
            'add_new_item' => __('Add new dashboard', 'odi'),
            'new_item' => __('New dashboard', 'odi'),
            'edit_item' => __('Edit dashboard', 'odi'),
            'view_item' => __('View dashboard', 'odi'),
            'all_items' => __('All dashboard', 'odi'),
            'search_items' => __('Search dashboard', 'odi'),
            'parent_item_colon' => __('Parent dashboard:', 'odi'),
            'not_found' => __('No dashboard found.', 'odi'),
            'not_found_in_trash' => __('No dashboard found in trash.', 'odi'),
            );
        ...
        function post_datavizs_box($post = false)
        {
          ...
          <p>
           <?php
            printf(__('Add and manage <a href="%s" target="_blank">Data visualizations</a> on your dashboard.', 'odi'), admin_url('edit.php?post_type=dataviz'));
          ...
          if($dataviz_query->have_posts()) {
           ...
           ?>
           <input type="text" data-bind="textInput: search" placeholder="<?php _e('Search for data visualizations', 'odi'); ?>" size="50">

           <!-- ko if: !search() -->
            <h4 class="results-title"><?php _e('Latest data visualizations', 'odi'); ?></h4>
           <!-- /ko -->
           ...
           <!-- ko if: !filteredDatavizs().length && !search() -->
            <p style="font-style:italic;color: #999;"><?php _e('You are using all of your data visualizations.', 'odi'); ?></p>
           <!-- /ko -->

           <!-- ko if: !filteredDatavizs().length && search() -->
            <p style="font-style:italic;color: #999;"><?php _e('No data visualizations were found.', 'odi'); ?></p>
           <!-- /ko -->
           ...
           <table class="datavizs-list available-datavizs">
            <tbody data-bind="foreach: filteredDatavizs">
             <tr>
              ...
              <td style="width:1%;"><a class="button" data-bind="click: $parent.addDataviz" href="javascript:void(0);" title="<?php _e('Add data visualization', 'odi'); ?>">+ <?php _e('Add'); ?></a></td>
             </tr>
            </tbody>
           </table>

           <h4 class="selected-title"><?php _e('Selected data visualizations', 'odi'); ?></h4>
            ...
            <tbody class="selected-datavizs-list">
             <!-- ko foreach: {data: selectedDatavizs} -->
              <tr class="dataviz-item">
               ...
               <td style="width:1%;"><a class="button" data-bind="click: $parent.removeDataviz" href="javascript:void(0);" title="<?php _e('Remove data visualization', 'odi'); ?>"><?php _e('Remove'); ?></a></td>
              </tr>
             <!-- /ko -->
            </tbody>
           </table>
